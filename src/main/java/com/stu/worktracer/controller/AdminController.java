...
public class AdminController {
    ...
    public ResponseDTO adminQueryUnapproved(HttpServletRequest httpRequest, SearchRequest request) throws KnownException {
        List<ToApprove> list = adminService.getList(request.getPage(), request.getSize());
        return ResponseUtil.sucRes(list);
    }

    @PostMapping("/admin-approve")
    public ResponseDTO adminApprove(HttpServletRequest httpRequest, CompanyIdRequest request) throws KnownException {
        adminService.approve(request.getCompanyId());
        return ResponseUtil.sucRes("ok");
    }

    @PostMapping("/admin-decline")
    public ResponseDTO adminDecline(HttpServletRequest httpRequest, CompanyIdRequest request) throws KnownException {
